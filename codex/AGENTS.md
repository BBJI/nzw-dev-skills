...
所有任务产出落到当前工作目录的 `.nds/` 下，状态记录在 `.nds/state.json`，支持跨会话续传。
...
- **req-analysis-skill**：把模糊想法转化为 PRD + 原型 HTML + 追溯矩阵 + 风险登记表。产出到 `.nds/01-requirements/`。
- **design-skill**：基于需求产出 W3C 标准设计令牌、组件规格、用户流程、高保真 HTML 稿。产出到 `.nds/02-design/`。
- **review-skill**：从需求完整性 / UX 可实现性 / 技术可行性三维度评审，产出 Issue/Risk/Decision + 准入签字。产出到 `.nds/03-review/`。
- **task-allocation-skill**：分解为 INVEST 合格、DAG 依赖、四要素契约的任务树。产出到 `.nds/04-tasks/`。
- **dev-skill**：TDD 红-绿-重构循环实现任务，Conventional Commits 提交。产出到 `.nds/05-dev/`。
- **test-skill**：测试用例设计（等价类/边界值/决策表/BDD Gherkin）+ 执行 + 缺陷报告 + 回归集。产出到 `.nds/06-test/`。
- **instruction-skill**：为 Claude Code / Codex / Cursor / Copilot / Windsurf 生成项目级规范文件。产出到 `.nds/00-instruction/`。
- **workflow-skill**：编排者，以 Plan→Act→Observe→Reflect→Iterate 闭环串联上述 7 阶段。产出到 `.nds/07-workflow/`。

## 执行约定

无论触发哪个 skill，都必须遵守：

1. **状态机驱动**：执行前读 `.nds/state.json`，执行后更新 `current_phase` / `phases.<phase>.status` / `resume_hint`，同步刷新 `.nds/PROGRESS.md`。
2. **目录契约**：各 skill 产出严格落入 `.nds/<编号-阶段名>/` 目录，不跨界。
3. **JSON+Markdown 双层**：机器态用 `state.json`，人类态用 `PROGRESS.md`，二者保持同步。
4. **人在环**：review 阶段必须等用户在 `.nds/03-review/sign-off.md` 签字才能进入开发。
5. **客观信号**：test/dev 阶段的 Observe 必须用真实测试/lint/编译输出，不依赖自评。
6. **跨会话续传**：任何阶段都可暂停，下次对话读 `state.json` 接续。
...
1. 读取 `.nds/state.json`
2. 报告：项目名、目标、当前阶段、各阶段状态、未完成任务、待修复 bug、`resume_hint`
3. 询问：继续当前阶段 / 跳转指定阶段 / 重做某阶段
4. 用户确认后触发对应 skill
...
9. 全部通过后调 instruction-skill 生成规范（新项目）
10. 标记 `current_phase = "done"`，向用户报告交付
